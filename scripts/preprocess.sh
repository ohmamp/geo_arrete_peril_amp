...
python src/preprocess/process_metadata.py ${DATA_INT}/${BATCH}_meta_base.csv ${DATA_INT}/${BATCH}_meta_proc.csv --append

# 3. extraire le texte natif des PDF ; 2 sorties: CSV de métadonnées enrichies + dossier pour les fichiers texte natif
python src/preprocess/extract_native_text.py ${DATA_INT}/${BATCH}_meta_proc.csv ${DATA_INT}/${BATCH}_meta_ntxt.csv ${DATA_INT} --append
...
python src/preprocess/determine_pdf_type.py ${DATA_INT}/${BATCH}_meta_ntxt.csv ${DATA_INT}/${BATCH}_meta_ntxt_pdftype.csv --append

# 4. rassembler les pages de texte natif dans un dataframe
python src/preprocess/separate_pages.py ${DATA_INT}/${BATCH}_meta_ntxt_pdftype.csv ${DATA_INT}/${BATCH}_ntxt_pages.csv --append

# 6. filtrer les documents qui sont hors périmètre (plan de périmètre de sécurité), et les annexes
python src/preprocess/filter_docs.py ${DATA_INT}/${BATCH}_meta_ntxt_pdftype.csv ${DATA_INT}/${BATCH}_ntxt_pages.csv ${DATA_INT}/${BATCH}_meta_ntxt_filt.csv ${DATA_INT}/${BATCH}_ntxt_pages_filt.csv --append

# 7. convertir les PDF natifs ("texte") en PDF/A
python src/preprocess/convert_native_pdf_to_pdfa.py ${DATA_INT}/${BATCH}_meta_ntxt_filt.csv ${DATA_INT}/${BATCH}_meta_ntxt_pdfa.csv ${DATA_INT} --append
...
python src/preprocess/extract_text_ocr.py ${DATA_INT}/${BATCH}_meta_ntxt_pdfa.csv ${DATA_INT}/${BATCH}_meta_otxt.csv ${DATA_INT} --append
